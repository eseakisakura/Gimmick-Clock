...
function Resize_Set(){ 
	...
	[int[]]$script:IMG= @($frm.Size.Width, ($frm.Size.Height- 40)) # グラフのサイズ
	...
	[int[]]$script:buff_size= @(($IMG[0]), ($IMG[1]))
	[int[]]$script:center= (($IMG[0]/ 2), ($IMG[1]/ 2))
...
class Slide_from_in{ 
	...
	Slide_from_in([int]$imgy){ # コンストラクタ
		$this.pos= $imgy
		$this.pix= $imgy/ 3
		$this.mem= "eject"
	...
	[int] eject(){
		...
		if($this.swh.ElapsedMilliseconds -ge 2000){
			$this.pos= $this.pix* 3
		}else{
			$this.pos+= $this.pix/ 2000* $this.swh.ElapsedMilliseconds
		}
		return $this.pos
	}
 } #class
...
class from_Watched_gimmick{ 
	...
	Timer(){
		...
		if($this.swh.ElapsedMilliseconds -ge ($this.milsec+ $this.milsec)){ # + $milsec safety
...
# read-host "pause"
			...
			$num[0]= $bai+ $this.elp[0]
			$num[1]= $bai+ $this.elp[1]
			$num[2]= $bai+ $this.elp[2]
			$num[5]= $baj+ $this.elp[5]
			...
			if($real[0] -le $this.elp[0] -and $this.mode_change[0] -ne "Watched"){ $real[0]+= $this.IMG[0] }
			if($real[1] -le $this.elp[1] -and $this.mode_change[0] -ne "Watched"){ $real[1]+= $this.IMG[0] }
			if($real[2] -le $this.elp[2] -or $this.mode_change[0] -eq "Watched"){ $real[2]+= $this.IMG[0] }
			if($real[5] -le $this.elp[5] -or $this.mode_change[0] -eq "Watched"){ $real[5]+= 360 }
			...
			$now[0]= [Math]::Floor(600/ $this.IMG[0]* $num[0]/ 50)% 12 # 600は固定値
			$now[1]= [Math]::Floor(600/ $this.IMG[0]* $num[1]/ 10)% 60
			$now[2]= [Math]::Floor(600/ $this.IMG[0]* $num[2]/ 10)% 60

			switch($this.mode_change[0]){ # wth -> wth / stopwth -> wth
			'Watched'{
				DrawTime $num $now $now
			}default{
				DrawStopWatch $num $now $now
			}
			} #sw
		}
	}
...
class from_StopWatched_gimmick{ 
	...
	Timer(){
		...
		if($this.swh.ElapsedMilliseconds -ge ($this.milsec+ $this.milsec)){ # + $this.milsec safety
			...
			if($real[0] -le $this.elp[0]){ $real[0]+= $this.IMG[0] } # 1cycle追加
			if($real[1] -le $this.elp[1]){ $real[1]+= $this.IMG[0] }
			if($real[2] -le $this.elp[2]){ $real[2]+= $this.IMG[0] }
			if($real[5] -le $this.elp[5]){ $real[5]+= 360 }
			...
			$now[0]= [Math]::Floor($this.IMG[0]/ $this.IMG[0]* $num[0]/ 50)% 12 # 600px
			$now[1]= [Math]::Floor($this.IMG[0]/ $this.IMG[0]* $num[1]/ 10)% 60
			$now[2]= [Math]::Floor($this.IMG[0]/ $this.IMG[0]* $num[2]/ 10)% 60
...
$buffs.Graphics.Clear($silver)
$buffs.Render($slide_graphics)
...
$slide_image= New-Object System.Drawing.Bitmap($IMG[0],($IMG[1]/ 3)) 
...
$contxt_slide.MaximumBuffer= @(($IMG[0]+ 2),($IMG[1]/ 2+ 2)) -join ","
$buffs= $contxt_slide.Allocate($slide_graphics, $slide_Pictbox.ClientRectangle)
...
$Slidein= New-Object Slide_from_in($buff_size[1]) 
...
$contxt_3.Text= "300px"
$contxt_3.Add_Click({ $frm.Size= @(300, 340) -join ","; Mode })

$contxt_6= New-Object System.Windows.Forms.ToolStripMenuItem
$contxt_6.Text= "600px"
$contxt_6.Add_Click({ $frm.Size= @(600, 640) -join ","; Mode })

$contxt_9= New-Object System.Windows.Forms.ToolStripMenuItem
$contxt_9.Text= "900px"
$contxt_9.Add_Click({ $frm.Size= @(900, 940) -join ","; Mode })
...
$contxt.Items.AddRange(@($contxt_size,$contxt_aset))
...
[array]$mode_change= "Watched","Watched",$False
